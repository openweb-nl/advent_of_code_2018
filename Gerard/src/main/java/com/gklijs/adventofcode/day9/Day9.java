...
public class Day9 {
    ...
    private static Triple<int[], List<Integer>, long[]> initialState(int[] input) {
        // first is the index of the current marble, second the id of the player about to play
        int[] trackers = new int[]{0, input[0]};
        List<Integer> marblesInPlay = new GapList<>(input[1]);
        marblesInPlay.add(0);
        long[] scores = new long[input[0]];
        return new Triple<>(trackers, marblesInPlay, scores);
    }

    private static void updateState(Triple<int[], List<Integer>, long[]> state, int marble) {
        ...
        if (state.getSecond().size() == 1) {
            ...
            int removedValue = (Integer) state.getSecond().remove(index);
    ...
    private static Triple<int[], List<Integer>, long[]> playGame(String input, int multiplier) {
        int[] playersAndMarbles = playersAndMarbles(input, multiplier);
        Triple<int[], List<Integer>, long[]> state = initialState(playersAndMarbles);
        IntStream.range(1, playersAndMarbles[1] + 1).forEach(marble -> updateState(state, marble));
        return state;
    }

    private static long heigestScore(Triple<int[], List<Integer>, long[]> input) {
